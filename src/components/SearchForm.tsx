...
const SearchForm = ({ compact = false }: SearchFormProps) => {
  ...
  const filteredDepartures = locations.filter((l) =>
    l.toLowerCase().includes(departure.toLowerCase())
  );
  const filteredDests = locations.filter((l) =>
    l.toLowerCase().includes(destination.toLowerCase())
  );
  ...
  return (
    <div className={`rounded-2xl bg-card p-4 shadow-lg ${compact ? '' : 'border border-border'}`}>
      {!compact && (
        <h2 className="mb-4 text-lg font-bold">🔍 Où allez-vous ?</h2>
      )}
      ...
      <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-3 py-2.5">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setShowDepartureSuggestions(e.target.value.length >= 1);
              }}
          ...
          <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-3 py-2.5">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setShowDestSuggestions(e.target.value.length >= 1);
              }}
        ...
        <motion.button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-mali py-3 text-sm font-bold text-primary-foreground shadow-mali transition-shadow hover:shadow-lg"
        >
